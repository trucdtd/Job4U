...
.container {
	margin-bottom: 1rem; /* Giảm khoảng cách giữa phần điều khoản và form đăng ký */
}
</style>
</head>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', function(event) {
            if (!termsAgreedCheckbox.checked) {
                event.preventDefault();  // Ngăn không cho submit form nếu điều khoản chưa được đồng ý
                const termsErrorModal = new bootstrap.Modal(document.getElementById('termsErrorModal'));
                termsErrorModal.show();  // Hiển thị modal thông báo lỗi
            }
        });
        ...
        if (successMessage) {
            const successModal = new bootstrap.Modal(document.getElementById('successModal'));
            successModal.show();  // Hiển thị modal thông báo thành công
        }
    });
</script>
