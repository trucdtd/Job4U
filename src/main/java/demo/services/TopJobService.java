...
public class TopJobService {
	...
	public Page<JoblistingsEntity> searchJobs(Optional<String> jobtitle, Optional<String> joblocation,
			Optional<String> industry, Pageable pageable) {
		Pageable sortedByDateDesc = PageRequest.of(pageable.getPageNumber(), pageable.getPageSize(),
				Sort.by("posteddate").descending());
		...
		if (jobtitle.isPresent() && !jobtitle.get().isEmpty()) {
			if (industry.isPresent() && !industry.get().isEmpty()) {
				if (joblocation.isPresent() && !joblocation.get().isEmpty()
					...
					return joblistingsDao.findByJobTitleAndJobLocationAndIndustry(jobtitle.get(), joblocation.get(),
							industry.get(), sortedByDateDesc);
				} else {
